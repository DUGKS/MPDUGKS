...
void Update_phi_Eq(Cell_2D &cell)
{
	...
	for(int j = 0;j < DV_Qv;++j)
	{
		cell.f.Eq[i][j] = vectorDot(iArkC[j],M16);
	}
}
void Update_phi_Eqh(Face_2D &face)
{
...
void IntegralShearStress()
{
	for(int n = 0;n < Cells;++n)
	{
		...
		for(int j = 0;j < DV_Qv;++j)
		{
			fNeq[j] = cell.fT[i][j] - cell.f.Eq[i][j];
		}
...
// {
// 	double _dx = face.owner->xc - face.neigh->xc;
// 	double _dy = face.owner->yc - face.neigh->yc;
// 	SetZero(_dx);
// 	SetZero(_dy);
...
// 		if(0.0 == _dx)
// 		{
// 			 _fBP_xF =  0.5*(face.owner->fBP_x[i][j] + face.neigh->fBP_x[i][j]);
// 			 _fBP_yF = (face.owner->fBP[i][j] - face.neigh->fBP[i][j])/_dy;
// 		}
// 		else if(0.0 == _dy)
// 		{
// 			_fBP_yF = 0.5*(face.owner->fBP_y[i][j] + face.neigh->fBP_y[i][j]);
// 			_fBP_xF = (face.owner->fBP[i][j] - face.neigh->fBP[i][j])/_dx;
// 		}
// 		else
// 		{
...
// 			getchar();
// 		}
// 		face.fBh[i][j] = 0.5*(face.owner->fBP[i][j] + face.neigh->fBP[i][j])
