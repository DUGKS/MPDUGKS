...
int MeshCheck()
{
	...
	LoopPS(Faces)
	{
		...
		if((face.Vx*VxCell + face.Vy*VyCell) <= 0)
		{
			_PRINT_SPLITLINE_ARK
			_PRINT_ERROR_MSG_FLIP
			cout << "Fatal Error : normal vector doesn't match with neighbour cells" << endl;
			getchar();
			_PRINT_SPLITLINE_ARK
			exit(-1);
		}
	...
	LoopPS(Faces)
	{
		...
		for(int k = 0;k < 4;++k)
		{
			x4 += face.faceCells[k]->xc;
			y4 += face.faceCells[k]->yc;
		}
		if(!doubleEqual(face.xf,0.25*x4))
		{
		...
		{
			cout <<"faceCells y coordinates doesn't match with the face"<<endl;
			cout <<"face.yf : "<<face.yf<<"    "<<"average yc : "<<0.25*y4<<endl;
			_PRINT_ERROR_MSG_FLIP
			getchar();
			exit(-1);
		}
